...
// =============================================================================
// MAP DATA - Bulk geo-tagged projects for map view
// =============================================================================
...
export interface GeoProject {
    ...
    copper_lbs?: number | null;
}

/**
 * Get all projects with coordinates for map display.
 * Single query with company join - replaces N+1 pattern.
 */
export async function getGeoProjects(): Promise<GeoProject[]> {
    const { data, error } = await supabase
        .from('projects')
        .select(`
            id,
    ...
    // Get production data for projects (if mine_production table has recent data)
    const projectIds = (data || []).map(p => p.id);
    let productionMap = new Map<number, { gold_oz?: number; silver_oz?: number; copper_lbs?: number }>();

    if (projectIds.length > 0) {
        // Get latest production for each project
        const { data: production } = await supabase
            .from('mine_production')
            .select('project_id, gold_oz, silver_oz, copper_lbs')
            .in('project_id', projectIds)
            .order('period_end', { ascending: false });

        // Group by project_id, take first (most recent)
        if (production) {
            for (const p of production) {
                if (!productionMap.has(p.project_id)) {
                    productionMap.set(p.project_id, {
                        gold_oz: p.gold_oz,
    ...
    // Transform to expected format
    return (data || []).map(p => {
        const production = productionMap.get(p.id);
        return {
            id: p.id,
            name: p.name,
            ...
            stage: p.stage,
            location: p.location,
            gold_oz: production?.gold_oz,
            silver_oz: production?.silver_oz,
            copper_lbs: production?.copper_lbs,
        };
    });
}
...
export interface PriceAlert {
    ...
    company_id: number;
    ticker: string;
    alert_type: 'price_above' | 'price_below' | 'change_percent_above' | 'change_percent_below' | 'volume_above' | '52w_high_near' | '52w_low_near';
...
export async function updateAlert(alertId: string, updates: Partial<PriceAlert>) {
    const { data, error } = await supabase
        .from('price_alerts')
        .update(updates)
        .eq('id', alertId)
...
export async function getAlertHistory(userId: string, limit: number = 50) {
    const { data, error } = await supabase
        .from('alert_history')
        .select(`
            *,
